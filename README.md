...
The current Windows download is `https://github.com/romanstma-cpu/rom-apps/releases/download/polybot-win-2.23.0/ROM.PolyBot-Setup-2.23.0.exe`, with its SHA-256 in `assets/SHA256SUMS-Polybot-2.23.0.txt`. The macOS 15 Apple Silicon download is attached to the `polybot-mac-20` prerelease, with its hash in `assets/POLYBOT-MAC-CHECKSUMS-2.23.0.txt`. Intel Mac builds are not distributed. Versioned filenames prevent stale browser caches from serving a different build under the same name. The corresponding source and release notes are maintained in the ROM Polybot desktop workspace; the legacy Python code under `polybot/` is not the source of this desktop release.
