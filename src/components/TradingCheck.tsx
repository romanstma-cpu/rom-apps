...
export function TradingCheck({ onOpenStrategy }: { onOpenStrategy: () => void }) {
  ...
  const facts = [
    ...
    { label: 'Recorded fees', value: fmtUsd(account?.feesUsd), detail: 'From recorded main-strategy orders' },
    { label: 'Position limit', value: fmtUsd(config?.hardMaxPositionUsd), detail: config ? `${Math.round(config.maxTotalExposureFraction * 100)}% portfolio cap` : 'Load strategy settings' },
    { label: 'Loss limit', value: dailyStop > 0 ? fmtUsd(dailyStop) : 'Off', detail: dailyStopped ? 'Daily stop has been reached' : 'Pauses new main-strategy entries' },
  ];
  ...
  return <section className={cls('rounded-2xl border p-5', attention ? 'border-rom-warn/40 bg-rom-warn/[0.045]' : 'border-rom-win/30 bg-rom-win/[0.035]')} aria-labelledby="trading-check-title">
    ...
    <dl className="mt-5 grid gap-x-6 gap-y-5 border-t border-rom-border pt-5 sm:grid-cols-2 xl:grid-cols-4">
      {facts.map((fact) => <div key={fact.label}>
        ...
        <dd className="mt-1 text-sm font-semibold tabular-nums text-white">{fact.value}</dd>
