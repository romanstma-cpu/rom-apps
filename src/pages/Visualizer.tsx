...
export function VisualizerPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const s = await window.rom.data.pnlSeries(24);
    ...
    for (const p of positions) {
      ...
      if (isOpen) {
        ...
        if (orb.stage !== 'orbit') {
          ...
          orb.color = p.signalSource === 'whale' ? '#3B82F6' : '#38BDF8';
        }
  ...
  useEffect(() => {
    ...
    const tick = (ts: number) => {
      if (document.hidden) {
        last = ts;
        hiddenTimer = window.setTimeout(() => { rafRef.current = requestAnimationFrame(tick); }, 500);
        return;
      }
  ...
  return (
    <Page
      ...
      actions={
        <>
          ...
          <button onClick={reset} className="rom-btn-default">
    ...
    >
      <div className="grid gap-4 lg:grid-cols-[320px,1fr]">
        <div className="space-y-3">
          ...
          <Card header={<div className="text-xs uppercase tracking-wider text-rom-muted">P&amp;L per hour (12h)</div>}>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 text-[11px] text-rom-muted">
          ...
          <Card header={<div className="text-xs uppercase tracking-wider text-rom-muted">Hot tickers (24h)</div>}>
            {hotTickers.length === 0 ? (
              <div className="py-3 text-center text-[11px] text-rom-dim">no resolutions yet</div>
            ) : (
              <div className="divide-y divide-rom-border">
                {hotTickers.map((t) => (
                  <div key={t.ticker} className="flex items-center gap-2 py-1.5 text-[11px]">
                    <span className="font-mono text-rom-muted truncate" title={t.ticker}>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <Card header={<div className="text-xs uppercase tracking-wider text-rom-muted"><Activity className="mr-1 inline h-3 w-3" />Latest signals</div>}>
          {signals.slice(0, 8).map((s) => (
            <div key={`${s.source}:${s.id}`} className="flex items-center gap-2 border-t border-rom-border py-1.5 first:border-t-0 text-xs">
              <span className="h-2 w-2 rounded-full" style={{ background: s.source === 'whale' ? '#3B82F6' : '#38BDF8' }} />
              <span className="font-mono text-rom-muted">{s.ticker}</span>
              <span className="ml-auto text-rom-dim truncate">{s.title}</span>
            </div>
          ))}
        </Card>
        <Card header={<div className="text-xs uppercase tracking-wider text-rom-muted"><Sparkles className="mr-1 inline h-3 w-3" />Latest trades</div>}>
...
function spawnScanOrb(
  ...
  return {
    ...
    color: source === 'whale' ? '#3B82F6' : '#38BDF8',
...
function drawOrb(ctx: CanvasRenderingContext2D, orb: Orb, cx: number, cy: number, ts: number) {
  ...
  if (lineAlpha > 0.02) {
    ...
    grad.addColorStop(0, withAlpha('#3B82F6', lineAlpha * 0.4));
...
function drawSweep(ctx: CanvasRenderingContext2D, cx: number, cy: number, ts: number, maxR: number) {
  ...
  ctx.strokeStyle = `rgba(59, 130, 246, ${0.18 * (1 - sweep)})`;
  ctx.lineWidth = 1;
  ctx.beginPath();
...
function drawSun(ctx: CanvasRenderingContext2D, cx: number, cy: number, t: number) {
  ...
  halo.addColorStop(0.4, 'rgba(59, 130, 246, 0.2)');
  ...
  return (
    <div className={cls('flex items-center gap-3 rounded-xl border px-4 py-3 backdrop-blur', bg)}>
      <div className={cls(
        ...
        tone === 'good' ? 'bg-rom-win/20' : 'bg-rom-loss/20',
      )}>
...
function TradeRow({ p }: { p: BotPosition }) {
  const tone = p.resolved
    ...
    : 'text-rom-muted';
  return (
    <div className="flex items-center gap-2 border-t border-rom-border py-1.5 first:border-t-0 text-xs">
      <span
        className="h-2 w-2 rounded-full"
        style={{ background: p.signalSource === 'whale' ? '#3B82F6' : '#38BDF8' }}
      />
      <span className="font-mono text-rom-muted">{p.ticker.split('-').pop() || p.ticker}</span>
      <span className="ml-auto truncate text-rom-dim">{p.title}</span>
