  ...
  return (
    ...
    >
      <div className="mb-3 grid grid-cols-1 items-center gap-2 md:grid-cols-[1fr_auto_auto_auto]">
        ...
        <label className="flex items-center gap-2 text-xs text-rom-muted">
          <input
            type="checkbox"
            checked={autoscroll}
            onChange={(e) => setAutoscroll(e.target.checked)}
            className="h-3.5 w-3.5 accent-rom-purple"
          />
          Auto-scroll
        </label>
      </div>
      ...
      <div
        ref={ref}
        className="h-[calc(100vh-260px)] overflow-y-auto rounded-xl border border-rom-border bg-black/50 p-3 font-mono text-[12px] leading-relaxed"
      >
        {filtered.length === 0 ? (
          <div className="grid h-full place-items-center text-xs text-rom-muted">
            No logs match. Backend may still be starting up.
          </div>
        ) : (
          filtered.map((l, i) => (
            <div key={i} className="flex gap-3 py-0.5">
              <span className="w-20 shrink-0 text-rom-dim">{fmtTimeShort(l.ts)}</span>
              <span className={cls('w-14 shrink-0 uppercase', LEVEL_COLORS[l.level] ?? '')}>
                {l.level}
              </span>
              <span className="w-20 shrink-0 truncate text-rom-purple">[{l.source}]</span>
