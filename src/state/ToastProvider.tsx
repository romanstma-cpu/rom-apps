...
function ToastItem({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  return (
    <div
      className={cls(
        'pointer-events-auto flex items-start gap-3 rounded-xl border bg-rom-surface/90 p-3 backdrop-blur-md shadow-rom-soft animate-fade-in',
        toast.kind === 'success' && 'border-rom-win/30',
        toast.kind === 'error' && 'border-rom-loss/40',
        toast.kind === 'warn' && 'border-rom-warn/40',
        toast.kind === 'info' && 'border-rom-border',
    ...
    >
      ...
      <button
        onClick={onClose}
        className="text-rom-muted hover:text-white"
        aria-label="Dismiss"
      >
        <X className="h-4 w-4" />
      </button>
