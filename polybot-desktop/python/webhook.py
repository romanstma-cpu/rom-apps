...
def _rom_footer(env: str, pos_id: int | str = "?", coid: str = "") -> dict:
    return {
        "text": f"ROM PolyBot · {env.upper()} · pos#{pos_id}"
...
def event_embed(kind: str, pos: dict, env: str, note: str = "") -> dict:
    ...
    embed: dict = {
        ...
        "timestamp": _now_iso(),
        "footer": _rom_footer(env, pos.get("id", "?"), pos.get("client_order_id", "")),
        "fields": [],
    }
...
def whale_embed(whale: dict) -> dict:
    ...
    return {
        ...
        "footer": {"text": "ROM PolyBot · Whale Scanner"},
    }
...
def momentum_embed(alert: dict) -> dict:
    ...
    return {
        ...
        "footer": {"text": "ROM PolyBot · Momentum Scanner"},
    }
...
async def send_event(url: str, kind: str, pos: dict, env: str, note: str = "") -> None:
    if not url:
        return
    payload = {"username": "ROM PolyBot", "embeds": [event_embed(kind, pos, env, note)]}
...
async def send_whale(url: str, whale: dict) -> None:
    if not url:
        return
    payload = {"username": "ROM PolyBot · Whale", "embeds": [whale_embed(whale)]}
    await _post(url, payload)
...
async def send_momentum(url: str, alert: dict) -> None:
    if not url:
        return
    payload = {"username": "ROM PolyBot · Momentum", "embeds": [momentum_embed(alert)]}
    await _post(url, payload)
...
async def send_startup(url: str, env: str, balance_usd: float, portfolio_usd: float) -> None:
    if not url:
        return
    embed = {
        "title": f"{EMOJI['startup']} ROM PolyBot online",
        "color": COLOR["startup"],
        "timestamp": _now_iso(),
        "description": (
        ...
        "footer": {"text": "rom.cc/tools/polybot"},
    }
...
async def send_shutdown(url: str, note: str = "clean exit") -> None:
    if not url:
        return
    embed = {
        "title": f"{EMOJI['shutdown']} ROM PolyBot offline",
...
def stats_embed(snap: dict, env: str) -> dict:
    ...
    return {
        "title": f"{EMOJI['stats']} ROM PolyBot · Hourly Stats",
        "color": color,
        "timestamp": _now_iso(),
        "description": (
        ...
        "footer": {"text": f"ROM PolyBot · {env.upper()} · Hourly Stats"},
    }
